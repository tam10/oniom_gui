...
public static class IcosphereGenerator {
	...
	public static void GenerateSphere(int resolution, Mesh mesh, Color32 color) {
		if (cachedGeometries.ContainsKey (resolution)) {
			...
			for (int c = 0; c < colors.Length; c++)
				colors [c] = color;
			SphereGeometry sphere = new SphereGeometry (triangles, verts, verts, colors);
			sphere.SetMesh (mesh);
	...
	private static void RefineFace(int faceIndex, List<Face> faces, List<Vector3> verts, Dictionary<long, int> cachedVertIndices) {
		...
		for (i0 = 0; i0 < 3; i0++) {
			...
			//Get vertex if it's in the cache, or calculate and add to cache
			int cachedInt;
...
class SphereGeometry {
	...
	public SphereGeometry(int[] triangles, Vector3[] vertices, Vector3[] normals, Color32[] colors) {
